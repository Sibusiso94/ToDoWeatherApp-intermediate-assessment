...
protocol WeatherViewModelManager {
    var weatherData: WeatherCachingModel? { get set }
...
class WeatherDomainManager: NSObject, CLLocationManagerDelegate, WeatherViewModelManager {
    let dataProvider = WeatherCacheManager()
    let dateManager = DateManager()
    let manager = CLLocationManager()
    var location: CLLocationCoordinate2D?
    
    var weatherData: WeatherCachingModel?
    var weatherInfo: WeatherDomainModel?
    var date: String = ""
    var time: String = ""
    var condition: ConditionIcon = .sunny
...
#warning("Move to manager")
    ...
    func fetchWeatherData(completion: () -> ()) {
